...
class UpdateManager(Manager):
    ...
    def _get_direct_dependencies_requirements(req_dev: bool) -> set:
        """Get all direct dependencies based on either requirements.in or requirements-dev.in file and generated Pipfile.lock from it."""
    ...
    def _get_requirements_txt_dependencies(req_dev: bool) -> dict:
        """Gather dependencies from either requirements.txt or requirements-dev.txt file, our requirements.txt and requirements-dev.txt holds fully pinned down stack."""
        result = {}
    ...
    def _create_update(self, dependency: str, package_version: str, old_version: str,
        ...
        # For either requirements.txt  or requirements-dev.text scenario we need to propagate all changes (updates of transitive dependencies)
        # into requirements.txt or requirements-dev file
        output_file = 'requirements-dev.txt' if req_dev else 'requirements.txt'
        self._pipenv_lock_requirements(output_file)
        merge_request = self._open_merge_request_update(
            dependency, old_version, package_version, labels, [input_file], merge_request
        )
        return old_version, package_version, merge_request.number
